...
export class ResourceCalendarHandlerService {
    ...
    async handleResourceCalendarPerUser(oauthUser: OAuth) {
        ...
        do {
            const calendars = await this.googleService.getResourcesCalendars(nextPageToken);
            for (const element of calendars.data.items) {
                let calendar = await this.resourceCalendarService.findOneByResourceId(element.resourceId);
                calendar = calendar ? calendar : new ResourceCalendar();
     ...
     * @param {OAuth} oauthUser
     */
    async subscribeToResourcesCalendar(calendar: ResourceCalendar, oauthUser: OAuth) {
        if (calendar.webHook) {
            return;
        }
        this.googleService.initialize(oauthUser);
        const uuid = uuidv4();
        const response = await this.googleService.watchCalendarEvents(calendar.resourceEmail, uuid);
