...
export class CalendarCommand {
     ...
     * @return void
     */
    @Command({command: 'download:events', describe: 'Download calendar events from google API & save to DB', autoExit: true})
    async events() {
        const resourceCalendars = await this.resourceCalendarService.findAll();
        for (const resourceCalendar of resourceCalendars) {
            await this.eventHandlerService.handleEventsPerResourceCalendar(resourceCalendar);
        }
    }
     ...
     * @return void
     */
    @Command({command: 'download:resource:calendars', describe: 'Download resource.calendar from google API & save to DB', autoExit: true})
    async resourceCalendars() {
        const oauthUsers = await this.oauthService.findAll();
        for (const oauthUser of oauthUsers) {
            await this.resourceCalendarHandlerService.handleResourceCalendarPerUser(oauthUser);
        }
    }
     ...
     * @return void
     */
    @Command({command: 'subscribe:resource:calendars', describe: 'Subscribe to all resource.calendar', autoExit: true})
