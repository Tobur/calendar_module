...
@Injectable()
export class ResourceCalendarService extends TypeOrmCrudService<
    ResourceCalendar
> {
    /**
    ...
    findOneByResourceId(resourceId: string): Promise<ResourceCalendar> {
        return this.resourceCalendarRepository.findOne({
            resourceId,
        });
    }
     ...
     * @return {ResourceCalendar}
     */
    findCalendarByCalendarIdAndUserId(
        calendarId: number,
        oauthId: number,
        ...
        return this.resourceCalendarRepository
            ...
            .where('q.id = :calendarId', { calendarId })
            .andWhere('oauth.id = :oauthId', { oauthId })
            .getOne();
    }
}
