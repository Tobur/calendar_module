...
export class ScheduleService {
    ...
    async handleOutDatedWebHook() {
        ...
        for (const webHook of webHooks) {
            ...
            this.logger.debug(
                `Update web hook for resource calendar id: ${calendar.id}`,
            );
            this.logger.debug(`Remove web hook id: ${webHook.id}`);
            await this.webHookService.remove(webHook.id);
            calendar.webHook = null;
            await this.resourceCalendarHandlerService.subscribeToResourcesCalendar(
                calendar,
                calendar.oauth,
            );
        }
    ...
    async handleChangedCalendars() {
        ...
        for (const webHook of webHooks) {
            this.logger.debug(
                'Sync calendar.',
                webHook.resourceCalendar.id.toString(),
            );
    ...
    /**
     * Download new resource calendars every 2 hours
     */
    @Cron(CronExpression.EVERY_2ND_HOUR)
